class SideMenu {
	...
	#setupEventListeners() {
		...
		document.addEventListener('keydown', (event) => {
			if (event.key === 'Escape' && this.sideMenuContainer.getAttribute('aria-hidden') === 'false') {
				this.#closeDrawer();
			}
		});
	}
	...
	#openDrawer() {
		...
		this.#toggleTabIndexAttr(this.closeSideMenuBtn, ...this.links);
	...
	#toggleTabIndexAttr(...elements) {
		elements.forEach((element) => {
			const tabIndexValue = element.getAttribute('tabindex');
			tabIndexValue === '-1' ? element.setAttribute('tabindex', '0') : element.setAttribute('tabindex', '-1');
		});
	}
}
