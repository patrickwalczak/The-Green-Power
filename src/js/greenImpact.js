class GreenImpactView {
  ...
  initEventListeners() {
    ...
    this.elements.dots.forEach((dot) =>
      dot.addEventListener('click', () =>
        this.gotoSlide(Number(dot.dataset.id))
      )
    );
  }
  ...
  updateSlideContent() {
    this.elements.descriptionContainer.innerHTML =
      this.descriptions[this.currSlide - 1];
    this.setActiveDot();
    this.updateBackground();
  }

  setActiveDot() {
    this.elements.dots.forEach((dot) => dot.classList.remove('dot__active'));
    this.elements.dots[this.currSlide - 1].classList.add('dot__active');
  }
  ...
  updateBackground() {
    ...
    const currentClass = [...this.elements.sectionSlider.classList].find(
      (cls) => cls.startsWith('slider__BG__')
    );
    if (currentClass)
      this.elements.sectionSlider.classList.remove(currentClass);
